...
# grab the first exon for all transcripts (rather than just one)
ex_tx <- exons(edb, filter = TxIdFilter(tx_ids), columns = c("tx_id","exon_idx"))
first_exons_tx <- ex_tx[ex_tx$exon_idx == 1]

# Keep standard chroms & harmonize naming
first_exons_tx <- keepStandardChromosomes(first_exons_tx, pruning.mode = "coarse")
seqlevelsStyle(first_exons_tx) <- seqlevelsStyle(Hs)

# drop any duplicates
key <- paste(seqnames(first_exons_tx), start(first_exons_tx), end(first_exons_tx), strand(first_exons_tx))
first_exons_tx <- first_exons_tx[!duplicated(key)]

# Get sequences
seqs <- Biostrings::getSeq(Hs, first_exons_tx)
names(seqs) <- paste0(
  gene_symbol, "|tx:", first_exons_tx$tx_id, "|",
  as.character(seqnames(first_exons_tx)), ":", start(first_exons_tx), "-", end(first_exons_tx),
  "(", as.character(strand(first_exons_tx)), ")"
)

## Output for use as a bulk search in Crackling
out_fa <- "CXCL10_exon1_all_tx.fa"
Biostrings::writeXStringSet(seqs, filepath = out_fa)
cat("Wrote:", length(seqs), "first-exon sequences to", out_fa, "\n")
...
# QC: 30–80% GC and no homopolymer >= 4
